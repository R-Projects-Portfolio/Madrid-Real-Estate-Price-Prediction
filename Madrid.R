...
#Our Regression model can't interpret Text data. 
#So we have to convert all text data into numeric columns,
...
#Changing names of columns so that they can be easily used in the model.
...
#Now we check all the NA's & NAN's & INF's one more time and replace them with 0.
...
#Now we remove the id & sq_ft_per_bedroom column because they don't add much to the regression.
...
#So we need to remove the columns we got from c_1 who have a correlation more than 0.7.
...
#Then we segregate the dependent and independent variables as y and x resp.
y <- final_df$S6
y 
x <- final_df[,-c(147)]
x
...
#Making the quantile regression model.
...
#So our AIC check reveal that for the best fit we need to remove column R6 & V6.
...
#So we see that our model still has columns which have p-values higher than 0.05 (significance value). 
...
#And model p value and predictors p value are less than 0.05, So our model is statistically significant.
